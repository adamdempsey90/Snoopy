...
double lastoutput_spec;								/**< Time when the last spectrum output was done */
...
void init_output() {
	...
	if(!param.restart) {
		...
		lastoutput_spec = param.t_initial - param.toutput_spec;
		lastoutput_flow = param.t_initial - param.toutput_flow;
		lastoutput_dump = param.t_initial - param.toutput_dump;
	
		init1Dspectrum();
		init_timevar();
		init1Dprofile(); //AJB 12/06/13
#ifdef FFTW_MPI_SUPPORT
		initHSpectrum();  //AMD 7/30/13
#endif
	}
...
void output(struct Field fldi, const double t) {
	...
	if( (t-lastoutput_spec)>=param.toutput_spec) {
		output_timer = output_timer - get_c_time();
		output1Dspectrum(fldi,t); //AJB
		lastoutput_spec = lastoutput_spec + param.toutput_spec;
		output1Dprofile(fldi,t); //AJB 12/06/13
		outputHSpectrum(fldi,t); //AMD 7/30/13
