...
/** 
	...
	Use Fortran output format (required by VTK). This routine is essentially
...
void write_vtk(FILE * ht, double complex wi[], const double t) {
	...
	//Input REAL field to output to vtk file. AJB
void write_vtk_real(FILE * ht, double wri[], const double t) {
	// Write the data in the file handler *ht
	int i,j,k;
	float q0;
...
// Geo's personal VTK writer, using structured data points	
/***********************************************************/
/** 
...
void output_vtk(struct Field fldi, const int n, double t) {
	...
	int array_size, i,j,k,maxind,indx,l;
	DEBUG_START_FUNC;

	sprintf(filename,OUTPUT_VTK_FILENAME,n);
#ifdef BOUNDARY_C
	...
	if(rank==0) {
		...
		fprintf(ht,"t= %015.15e \n",t);//AJB comment Snoopy Code v5.0\n",t);
		fprintf(ht, "BINARY\n");
		fprintf(ht, "DATASET STRUCTURED_POINTS\n");
#ifdef BOUNDARY_C //AJB
		fprintf(ht, "DIMENSIONS %d %d %d\n", NX, NY, NZ / 2);
#else
		fprintf(ht, "DIMENSIONS %d %d %d\n", NX, NY, NZ);
#endif
#ifdef BOUNDARY_C //AJB
		fprintf(ht, "ORIGIN %g %g %g\n", -param.lx/2.0,-param.ly/2.0,0.0);
#else
	...
	if(param.output_vorticity) {
		...
		for( i = 0 ; i < NTOTAL_COMPLEX ; i++) {
			w4[i] = I * (kyt[i] * fldi.vz[i] - kzt[i] * fldi.vy[i]);
			w5[i] = I * (kzt[i] * fldi.vx[i] - kxt[i] * fldi.vz[i]);
			w6[i] = I * (kxt[i] * fldi.vy[i] - kyt[i] * fldi.vx[i]);
		}
