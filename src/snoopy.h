...
struct Parameters {
  ...
  //#endif
	...
	int    antialiasing;		/**< 2/3 Antialisaing rule. Could be removed if you assume is unimportant (untested feature). */
        ...
        int    spectralfilter;	  //AJB spectral filter of Hou & Li - only works with same LX and resolution in each dimension
	...
	// Output parameters
        ...
        double toutput_spec;		/**< Time between two outputs in the timevar file */
	...
	double vortex_b;			/**< y dimension of the vortex */
        ...
        int    init_Kelvin_Wave; //AJB, add Kelvin Wave to box
        long   Kelvin_Wave_i;
        long   Kelvin_Wave_j; 
        long   Kelvin_Wave_k;
        double Kelvin_Wave_amp;

        int    init_B_field;
        double B_amp;

        int    init_Lamb_Vortex; //AJB Lamb-Oseen Vortex
        double Lamb_Vortex_vorticity;
        double Lamb_Vortex_lambda;
        double Lamb_Vortex_aspect;
	...
	double noise_cut_length;		/**< Wavelength over which the noise is applied */
        double noise_cut_length_max;		/**< Wavelength over which the noise is applied */	

	int    init_large_scale_noise_k;	/**< Init a large scale random noise */
        double kmin;		/**< Wavenumbers over which the noise is applied */
        double kmax;		/**< Wavenumbers over which the noise is applied */
