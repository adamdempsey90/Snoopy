...
void read_config() {
	...
	if(rank==0) {
...
#ifndef WITH_ELLIPTICAL_VORTEX
		param.gamma = 0.0;
		param.epsilon = 0.0;
#endif
		if(!config_lookup_float(&config, "physics.gamma",&param.gamma)) {
			param.gamma = 0.0;
		}
		if(!config_lookup_float(&config, "physics.epsilon",&param.epsilon)) {
			param.epsilon = 0.0;
		} 
		//AJB
		if(!config_lookup_float(&config, "physics.zc",&param.zc)) {
		  param.zc = 0.0;
		}
		if(!config_lookup_float(&config, "physics.delz",&param.delz)) {
		  param.delz = 0.0;
		}
		if(!config_lookup_float(&config, "physics.alpha",&param.alpha)) {
		  param.alpha = 0.0;
		}
		if(!config_lookup_float(&config, "physics.theta",&param.theta)) {
		  param.theta = 0.0;
		}
		if(!config_lookup_bool(&config, "physics.hyperdiffusion",&param.hyperdiffusion)) {
		  param.hyperdiffusion = 0;
		}
		if(!config_lookup_bool(&config, "physics.fluxrelax",&param.fluxrelax)) {
		  param.fluxrelax = 0;
		}
		if(!config_lookup_bool(&config, "physics.heating",&param.heating)) {
		  param.heating = 0;
		}
		if(!config_lookup_bool(&config, "physics.nonlinearoff",&param.nonlinearoff)) {
		  param.nonlinearoff = 0;
		}
		if(!config_lookup_bool(&config, "physics.numericalkevol",&param.numericalkevol)) {
		  param.numericalkevol = 0;
		}
		//AJB
		...
		if(!config_lookup_bool(&config, "code.spectralfilter",&param.spectralfilter)) {
		        param.spectralfilter = 0;
		}
		...
		if(!config_lookup_float(&config, "output.spectrum_step",&param.toutput_spec)) {
		  param.toutput_spec = 1.0;
		} //AJB
	...
	/* 	if(!config_lookup_string(&config, "output.filepath",&param.filepath)) { */
/* 		  param.filepath = "";; */
/* 		} */
		...
		if(!config_lookup_bool(&config, "init.B_field.enable",&param.init_B_field)) {
			param.init_B_field = 0;
		}
		if(!config_lookup_float(&config, "init.B_field.amplitude",&param.B_amp)) {
			param.B_amp = 0;
		}
		if(!config_lookup_bool(&config, "init.Kelvin_Wave.enable",&param.init_Kelvin_Wave)) {
			param.init_Kelvin_Wave = 0;
		}
		if(!config_lookup_int(&config, "init.Kelvin_Wave.i",&param.Kelvin_Wave_i)) {
		  param.Kelvin_Wave_i = 0;
		}
		if(!config_lookup_int(&config, "init.Kelvin_Wave.j",&param.Kelvin_Wave_j)) {
		  param.Kelvin_Wave_j = 0;
		}
		if(!config_lookup_int(&config, "init.Kelvin_Wave.k",&param.Kelvin_Wave_k)) {
		  param.Kelvin_Wave_k = 0;
		}
		if(!config_lookup_float(&config, "init.Kelvin_Wave.amp",&param.Kelvin_Wave_amp)) {
		  param.Kelvin_Wave_amp = 0;
		}
		if(!config_lookup_bool(&config, "init.Lamb_Vortex.enable",&param.init_Lamb_Vortex)) {
			param.init_Lamb_Vortex = 0;
		}
		if(!config_lookup_float(&config, "init.Lamb_Vortex.vorticity",&param.Lamb_Vortex_vorticity)) {
			param.Lamb_Vortex_vorticity = 0;
		}
		if(!config_lookup_float(&config, "init.Lamb_Vortex.lambda",&param.Lamb_Vortex_lambda)) {
			param.Lamb_Vortex_lambda = 0;
		}
		if(!config_lookup_float(&config, "init.Lamb_Vortex.aspect",&param.Lamb_Vortex_aspect)) {
			param.Lamb_Vortex_aspect = 0;
		}
		...
		if(!config_lookup_float(&config, "init.large_scale_noise.cut_length_max",&param.noise_cut_length_max)) {
			param.noise_cut_length_max = 0.0;
		}
		...
		if(!config_lookup_bool(&config, "init.large_scale_noise_k.enable",&param.init_large_scale_noise_k)) {
			param.init_large_scale_noise_k = 0;
		}
		if(!config_lookup_float(&config, "init.large_scale_noise_k.kmin",&param.kmin)) {
		        param.kmin = 0.0;
		}
		if(!config_lookup_float(&config, "init.large_scale_noise_k.kmax",&param.kmax)) {
		        param.kmax = 0.0;
		}	
		if(!config_lookup_float(&config, "init.large_scale_noise_k.amplitude",&param.per_amplitude_large)) {
		        param.per_amplitude_large = 0.0;
		}
