...
void symm_sin_z(double complex wi[]) {
		...
		if(rank==0) {
			for( i = 0; i <= NX_COMPLEX / 2; i++) {
				...
				for( j = 0; j < NY_COMPLEX; j++) {
					...
					//	MPI_Printf("%08e\t,%08e\t,%08e\t,%08e\t,%08e\t \n",((double) k),((double) j+i*NY_COMPLEX),((double) jtarget+itarget*NY_COMPLEX),creal(q0),cimag(q0));  //AJB				       
 ...
 /* if(k + j * NZ_COMPLEX + i * NZ_COMPLEX * NY_COMPLEX==1) { */
 /*   MPI_Printf("%08e\t,%08e\t \n",((double) k + jtarget * NZ_COMPLEX + itarget * NZ_COMPLEX * NY_COMPLEX),cabs(q1));} */ //AJB
			  q0 = 0.5*(q0-q1);
